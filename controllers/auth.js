...
exports.getVerifyEmail = (req, res, next) => {
  ...
  Users.findOne({ username: req.user.username }).then((user) => {
    const verification_token = randomstring.generate({ length: 10 });
    ...
    const mainOptions = {
      ...
      text: "text ne",
      html:
        "<p>Cảm ơn đã đăng kí tài khoản của woieeshop. Mã kích hoạt của bạn là:</p>" +
        verification_token,
    };
...
exports.postForgotPass = (req, res, next) => {
  ...
  Users.findOne({ email: email }, (err, user) => {
    if (!user) {
      ...
      const tpass = randomstring.generate({ length: 6 });
